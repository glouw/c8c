...
static node* add(node* nodes, node* new)
{
    ...
    if(cmp == 0); // do nothing
    else if(cmp < 0) nodes->less = add(nodes->less, new);
    else nodes->more = add(nodes->more, new);
    return nodes;
}
...
static node* get(node* nodes, char* token)
{
    ...
    if(cmp == 0) return nodes;
    else if(cmp < 0) return get(nodes->less, token);
    else return get(nodes->more, token);
}
